// "Songs to aim for" — real, on-the-radio songs a beginner can play with the open chords.
//
// IMPORTANT: these are COPYRIGHTED. We never print their lyrics or a chord-over-lyric chart
// here — only the song's name/artist, uncopyrightable musical facts, original beginner coaching,
// and links to licensed/outside instruction. The bundled *play-along* songbook stays public-domain
// only. Chords/capo below are a common beginner arrangement; the recording's exact key may
// differ, so the "Find the chords" link is the source of truth.

export const TARGET_SONGS = [
  // ----- Vocal showcases (for a strong singer) -----
  { title: 'Fast Car', artist: 'Tracy Chapman', genres: ['folk', 'americana'],
    chords: ['Cadd9', 'G', 'Em', 'D'], capo: 'Capo 2',
    why: 'A storytelling masterclass — four repeating chords, all about the voice.' },
  { title: 'Landslide', artist: 'Fleetwood Mac', genres: ['folk', 'americana'],
    chords: ['C', 'G', 'Am'], capo: 'Capo 3',
    why: 'Timeless, intimate, and built to carry a great vocal.' },
  { title: 'Hallelujah', artist: 'Leonard Cohen', genres: ['folk', 'church'],
    chords: ['C', 'Am', 'Fmaj7', 'G', 'E'], capo: 'No capo',
    why: 'The ultimate slow-build vocal showcase, using beginner-friendly Fmaj7.' },
  { title: 'Girl Crush', artist: 'Little Big Town', genres: ['country'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo — to taste',
    why: 'A slow-burn country vocal that lets a singer soar.' },
  { title: 'House of the Rising Sun', artist: 'The Animals', genres: ['folk', 'americana'],
    chords: ['Am', 'C', 'D', 'Fmaj7', 'E'], capo: 'No capo',
    why: 'The traditional ballad became an iconic recorded arrangement; learn that specific version from a licensed chart rather than mixing it into Campfire’s public-domain songbook.' },

  // ----- Sing with the kids -----
  { title: 'You Are My Sunshine', artist: 'Jimmie Davis (traditional country)', genres: ['kids', 'country'],
    chords: ['G', 'C', 'D'], capo: 'No capo',
    why: 'Maybe the ultimate parent-and-kid singalong. Three chords.' },
  { title: 'This Land Is Your Land', artist: 'Woody Guthrie', genres: ['kids', 'folk', 'americana'],
    chords: ['G', 'C', 'D'], capo: 'No capo',
    why: 'A joyful family singalong the whole car can belt out.' },
  { title: 'Rainbow Connection', artist: 'Kermit the Frog (Paul Williams)', genres: ['kids'],
    chords: ['G', 'Em', 'C', 'D'], capo: 'No capo',
    why: 'Sweet, gentle, and a little magic — great for winding down.' },

  // ----- Current hits (requested) -----
  { title: 'You Look Like You Love Me', artist: 'Ella Langley (feat. Riley Green)', genres: ['country'],
    chords: ['G', 'Am', 'C', 'D'], capo: 'Capo — follow tutorial',
    why: 'Her breakout duet — a modern honky-tonk singalong.',
    tutorial: 'https://www.youtube.com/watch?v=PwenPRaFMjY',
    bridge: { groove: 'Modern honky-tonk: steady country pulse with room for the spoken verses.',
      firstStep: 'Learn Am after G, C, and D; then practice G→Am and C→D as separate two-chord loops.',
      lead: 'Accompany first. Save any country fill for the turnaround after a vocal phrase.' } },
  { title: "Weren't For The Wind", artist: 'Ella Langley', genres: ['country', 'americana'],
    chords: ['Am', 'C', 'G', 'D', 'Em'], capo: 'Capo 2',
    why: 'A slow, aching ballad — great for practicing smooth changes.',
    tutorial: 'https://www.youtube.com/watch?v=D_G16hlnniI',
    bridge: { groove: 'Even eighth-notes: D-U-D-U-D-U-D-U, kept quiet and controlled.',
      firstStep: 'Loop Am→Em→G→D slowly; add the C→Em chorus move only after that feels automatic.',
      lead: 'The continuous acoustic rhythm is the hook. Do not add lead until the strum stays even under a singer.' } },
  { title: 'Last Night', artist: 'Morgan Wallen', genres: ['country'],
    chords: ['Cadd9', 'D', 'Em7'], capo: 'Tune ½-step down · no capo',
    why: 'His biggest hit — a repeating three-chord acoustic loop.',
    tutorial: 'https://www.learnguitarfavorites.com/blog/last-night',
    bridge: { groove: 'Continuous eighth-notes: D-U-D-U-D-U-D-U, with a light wrist.',
      firstStep: 'Use the familiar C and Em shapes as Cadd9 and Em7, then loop Cadd9→D→Em7 without stopping.',
      lead: 'The repeating acoustic figure already fills the lead role; prioritize its pulse under the vocal.' } },
  { title: 'Whiskey Glasses', artist: 'Morgan Wallen', genres: ['country'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo — to taste',
    why: 'A barroom singalong everybody knows.' },
  { title: 'Wasted on You', artist: 'Morgan Wallen', genres: ['country'],
    chords: ['G', 'D', 'Em', 'C'], capo: 'Capo — to taste',
    why: 'Big and emotional, and only four chords.' },
  { title: 'Tennessee Orange', artist: 'Megan Moroney', genres: ['country'],
    chords: ['G', 'Em', 'Am', 'C', 'D'], capo: 'Beginner chart in G',
    why: 'A conversational vocal over familiar open shapes, with enough movement for an intermediate goal.',
    tutorial: 'https://www.youtube.com/watch?v=-eRJ2hREL_s',
    bridge: { groove: 'Slow compound country feel: count two large pulses instead of six heavy strokes.',
      firstStep: 'Loop G→Em→Am→C, then separately practice the Am→D→G resolution before combining sections.',
      lead: 'Let the vocal phrasing lead. Keep the verse sparse and save the larger brushes for the chorus.' } },
  { title: 'I Never Lie', artist: 'Zach Top', genres: ['country'],
    chords: ['D', 'G', 'A', 'Bm', 'Em'], capo: 'Tutorial uses open D-family shapes plus Bm',
    why: 'A real bridge from open-chord rhythm into a first useful barre chord and classic-country phrasing.',
    tutorial: 'https://www.youtube.com/watch?v=NclRM4MW8zk',
    bridge: { groove: 'Classic country down-up pulse with deliberate single-strum stops at phrase endings.',
      firstStep: 'Master D→G→A first. Add Em, then treat Bm as the intermediate test rather than rushing it.',
      lead: 'Learn the short intro only after the verse changes stay in time; return to simple backing under the vocal.' } },

  // ----- Modern / classic country -----
  { title: 'Wagon Wheel', artist: 'Old Crow Medicine Show / Darius Rucker', genres: ['country', 'americana'],
    chords: ['G', 'D', 'Em', 'C'], capo: 'No capo',
    why: 'The ultimate 4-chord country singalong. If you learn one, learn this.' },
  { title: 'Take Me Home, Country Roads', artist: 'John Denver', genres: ['country', 'americana'],
    chords: ['G', 'Em', 'D', 'C'], capo: 'No capo',
    why: 'Everyone in the room knows every word.' },
  { title: 'Ring of Fire', artist: 'Johnny Cash', genres: ['country'],
    chords: ['G', 'C', 'D'], capo: 'No capo',
    why: 'Three chords and a horn line you can hum.' },
  { title: 'Chicken Fried', artist: 'Zac Brown Band', genres: ['country'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo 2',
    why: 'Feel-good and forgiving on the chord changes.' },
  { title: 'Jolene', artist: 'Dolly Parton', genres: ['country'],
    chords: ['Am', 'C', 'G'], capo: 'Capo 4',
    why: 'Mostly one hypnotic riff — a perfect first minor-key jam.' },
  { title: 'Tennessee Whiskey', artist: 'Chris Stapleton', genres: ['country', 'americana'],
    chords: ['G', 'Am'], capo: 'Capo 2',
    why: 'Basically two chords — it\'s all about smooth, slow changes.' },
  { title: 'Beautiful Crazy', artist: 'Luke Combs', genres: ['country'],
    chords: ['G', 'Em', 'C', 'D'], capo: 'Capo 1',
    why: 'The modern country slow-dance.' },
  { title: 'Something in the Orange', artist: 'Zach Bryan', genres: ['country', 'americana'],
    chords: ['G', 'D', 'Em', 'C'], capo: 'No capo',
    why: 'Slow, spacious, and only four chords.' },
  { title: 'Feathered Indians', artist: 'Tyler Childers', genres: ['americana', 'country'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo — to taste',
    why: 'A modern americana favorite built on the same four friends.' },

  // ----- Contemporary worship -----
  { title: '10,000 Reasons (Bless the Lord)', artist: 'Matt Redman', genres: ['church'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo 1',
    why: 'The gentlest on-ramp to worship guitar.',
    tutorial: 'https://www.worshiptogether.com/videos/10-000-reasons-bless-the-lord/',
    bridge: { groove: 'Gentle 4/4 worship pulse; begin with one down-strum per beat before adding light up-strums.',
      firstStep: 'Practice G, C, D, and Em in pairs, then follow the licensed lesson for section order.',
      lead: 'Let the congregation carry the melody. Use open strings as texture only between lines.' } },
  { title: 'Great Are You Lord', artist: 'All Sons & Daughters', genres: ['church'],
    chords: ['G', 'C', 'D', 'Em'], capo: 'Capo 2',
    why: 'Four chords with lots of room to breathe.',
    tutorial: 'https://www.youtube.com/watch?v=7rspK7mZl5Q',
    bridge: { groove: 'Slow 6/8 at about 72 BPM: feel two large pulses, each divided into three.',
      firstStep: 'Count “ONE-two-three FOUR-five-six” on one chord before adding the progression.',
      lead: 'Stay spacious under the vocal; a high-string swell or short answer belongs only after a held phrase.' } },
  { title: 'How Great Is Our God', artist: 'Chris Tomlin', genres: ['church'],
    chords: ['G', 'Em', 'C', 'D'], capo: 'Capo — to taste',
    why: 'A modern hymn the whole room sings.',
    tutorial: 'https://app.theworshipinitiative.com/craft-training/lessons/2-7-how-great-is-our-god',
    bridge: { groove: 'Steady 4/4 worship strum with quieter verses and a wider chorus.',
      firstStep: 'Campfire already builds G, Em, C, and D; loop each change at 60 BPM before following the lesson.',
      lead: 'Lead guitar should echo the melody only in section transitions; acoustic accompaniment stays simple.' } },
  { title: 'Build My Life', artist: 'Pat Barrett', genres: ['church'],
    chords: ['G', 'C', 'Em', 'D'], capo: 'Capo — to taste',
    why: 'A four-chord worship anthem.' },
];

// A neutral web search for the full chart (we don't bundle or endorse a specific chords site).
export function chartSearchUrl(t) {
  return 'https://www.google.com/search?q=' + encodeURIComponent(`${t.title} ${t.artist} guitar chords`);
}
